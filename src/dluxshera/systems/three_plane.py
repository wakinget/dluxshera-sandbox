...
class SheraThreePlaneConfig(BaseConfig):
    ...
    *numeric state* of an inference run (binary separation, flux, Zernike
    ...
    basis using these indices and will expect `optics.primary.zernike_coeffs_nm` in
    ...
    under `optics.secondary.zernike_coeffs_nm`. Forward-model specs built from this
...
def build_forward_spec_from_config(cfg: SheraThreePlaneConfig) -> ParamSpec:
    ...
    optics_kind = str(optics_overrides.get("kind", "three_plane")) if isinstance(optics_overrides, dict) else "three_plane"
    system_cfg = {
