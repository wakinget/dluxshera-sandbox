...
CONFIG_FIELD_REGISTRY: dict[str, dict[str, Any]] = {
    ...
    "experiment.detector_calibration_knowledge_error.apply_to": _entry(
        valid_values=_values(
            ("inference", "implemented", "Patch detector calibration knowledge error into the inference/reference detector template only."),
            ("truth", "implemented", "Patch detector calibration knowledge error into the truth/render detector template only."),
            ("render", "implemented", "Alias for truth/render detector template patching."),
            ("both", "implemented", "Patch both truth/render and inference/reference detector templates."),
        ),
        default="inference",
        consumed_by="detector_calibration_knowledge_error/campaign_model_split",
        runtime_effect="selects which detector template receives calibration-map knowledge-error layers",
        fidelity_effect="truth/reference detector calibration mismatch",
        provenance_effect="recorded in detector calibration KE provenance",
    ),
    "experiment.detector_calibration_knowledge_error.realization_policy": _entry(
        valid_values=_values(
            ("fixed_per_experiment", "implemented", "Use one fixed detector calibration KE realization for the campaign model split."),
            ("per_run", "implemented", "Parsed but rejected by full-fidelity campaign-level model split; use fixed_per_experiment here."),
        ),
        default="fixed_per_experiment",
        consumed_by="detector_calibration_knowledge_error/campaign_model_split",
        runtime_effect="controls detector calibration KE realization scope",
        fidelity_effect="static calibration mismatch policy",
        provenance_effect="recorded in detector calibration KE provenance",
        notes="per_run is recognized so validation can produce a domain error, but campaign model split currently rejects it.",
    ),
    "experiment.detector_calibration_knowledge_error.pixel_offsets.distribution": _entry(
        valid_values=_values(("normal", "implemented", "Gaussian pixel-offset map perturbations."), ("gaussian", "implemented", "Compatibility spelling normalized to normal.")),
        default="normal",
        consumed_by="detector_calibration_knowledge_error",
        runtime_effect="selects the pixel-offset calibration error distribution",
        fidelity_effect="pixel offset calibration mismatch",
        provenance_effect="normalized request recorded in detector calibration KE provenance",
    ),
    "experiment.detector_calibration_knowledge_error.pixel_response.distribution": _entry(
        valid_values=_values(("normal", "implemented", "Gaussian pixel-response map perturbations."), ("gaussian", "implemented", "Compatibility spelling normalized to normal.")),
        default="normal",
        consumed_by="detector_calibration_knowledge_error",
        runtime_effect="selects the pixel-response calibration error distribution",
        fidelity_effect="flat-field/pixel-response calibration mismatch",
        provenance_effect="normalized request recorded in detector calibration KE provenance",
    ),
    ...
    "experiment.subblocks.trajectory_processing.smear.edge_policy": _entry(valid_values=_values(("error", "implemented", "Fail when an exposure interval leaves the trajectory domain."), ("clamp", "implemented", "Clamp out-of-domain exposure endpoints to the trajectory domain."), ("drop", "implemented", "Disable smear rows for out-of-domain exposure intervals."), ("symmetric_linear_extrapolate", "implemented", "Linearly extrapolate leading/trailing endpoints from the nearest trajectory segment."), ("linear_extrapolate", "implemented", "Alias for symmetric endpoint linear extrapolation."), ("nearest_segment_extrapolate", "implemented", "Alias for symmetric endpoint linear extrapolation.")), default="error", consumed_by="trajectory_smear", runtime_effect="controls endpoint exposure handling for smear derivation", fidelity_effect="preserves centered endpoint smear kernels when extrapolation is enabled", provenance_effect="recorded in smear provenance"),
    "experiment.subblocks.trajectory_processing.smear.max_extrapolation_s": _entry(default="auto", consumed_by="trajectory_smear", runtime_effect="limits endpoint linear extrapolation duration", fidelity_effect="guards against far outside-domain smear requests", provenance_effect="recorded with resolved leading/trailing limits"),
    "experiment.subblocks.trajectory_processing.smear.exposure.edge_policy": _entry(valid_values=_values(("error", "implemented", "Fail when an exposure interval leaves the trajectory domain."), ("clamp", "implemented", "Clamp out-of-domain exposure endpoints to the trajectory domain."), ("drop", "implemented", "Disable smear rows for out-of-domain exposure intervals."), ("symmetric_linear_extrapolate", "implemented", "Linearly extrapolate leading/trailing endpoints from the nearest trajectory segment."), ("linear_extrapolate", "implemented", "Alias for symmetric endpoint linear extrapolation."), ("nearest_segment_extrapolate", "implemented", "Alias for symmetric endpoint linear extrapolation.")), default="error", consumed_by="trajectory_smear", runtime_effect="overrides trajectory_processing.smear.edge_policy for exposure endpoint handling", fidelity_effect="preserves centered endpoint smear kernels when extrapolation is enabled", provenance_effect="recorded in smear provenance"),
    "experiment.subblocks.trajectory_processing.smear.exposure.max_extrapolation_s": _entry(default="auto", consumed_by="trajectory_smear", runtime_effect="overrides trajectory_processing.smear.max_extrapolation_s", fidelity_effect="guards against far outside-domain smear requests", provenance_effect="recorded with resolved leading/trailing limits"),
