...
- This module owns block resolution (preset merge) and lightweight structural
  validation. Field-aware numeric normalization lives in ``config.numeric`` and
  in the consumers that know the relevant schema.
...
_SYSTEM_OPTICS_KEYS = {
    ...
    "diffractive_pupil_path",
    "dp_path",
    "dp_design_wavelength_m",
    "high_order_wfe",
}

_SYSTEM_DETECTOR_KEYS = {
    "model",
    "layers",
}


def _warn_unknown_system_keys(system: Mapping[str, Any]) -> None:
    """Warn on unknown top-level component keys in the resolved system block.

    This stays intentionally lighter than full component schema validation:
    nested detector layer options and high-order WFE maps are delegated to their
    builders, while obvious top-level typos are made visible before contracts
    silently ignore them.
    """

    block_specs = {
        "source": _SYSTEM_SOURCE_KEYS,
        "optics": _SYSTEM_OPTICS_KEYS,
        "detector": _SYSTEM_DETECTOR_KEYS,
    }
    for block_name, allowed in block_specs.items():
        block = system.get(block_name)
        if not isinstance(block, Mapping):
            continue
        for key in sorted(set(block) - allowed):
            warnings.warn(
                f"Unknown config key preserved for downstream handling: system.{block_name}.{key}",
                UserWarning,
                stacklevel=3,
            )
...
def _validate_system_schema(system: object) -> None:
    ...
    _warn_unknown_system_keys(system)
