...
def SheraThreePlane_ForwardModel(params, return_model=False, mask=_UNSET):
    ...
    detector = dl.LayeredDetector(layers=[("downsample", dl.Downsample(model_optics.oversample))])
...
class SheraThreePlane_Model(dl.Telescope):
    ...
    def _initialize_optics(self, params, mask=_UNSET):
        ...
        m1_cal_wfe = oneoverf_noise_2D(
            optics.wf_npixels, params.get("m1_calibrated_power_law"), key=subkey
        )
        m1_cal_wfe = remove_PTT(
            m1_cal_wfe, optics.m1_aperture.transmission.astype(bool)
        )  # Remove PTT from aperture
        m1_cal_wfe = m1_cal_wfe * (
            params.get("m1_calibrated_amplitude")
            / nanrms(m1_cal_wfe[optics.m1_aperture.transmission.astype(bool)])
        )  # Scale the 1/f noise map over the aperture
        ...
        m2_cal_wfe = oneoverf_noise_2D(
            optics.wf_npixels, params.get("m2_calibrated_power_law"), key=subkey
        )
        m2_cal_wfe = remove_PTT(
            m2_cal_wfe, optics.m2_aperture.transmission.astype(bool)
        )  # Remove PTT from aperture
        m2_cal_wfe = m2_cal_wfe * (
            params.get("m2_calibrated_amplitude")
            / nanrms(m2_cal_wfe[optics.m2_aperture.transmission.astype(bool)])
        )  # Scale the 1/f noise map over the aperture
        ...
        # First for the Primary
        rng_key, subkey = jr.split(rng_key)
        m1_uncal_wfe = oneoverf_noise_2D(
            optics.wf_npixels, params.get("m1_uncalibrated_power_law"), key=subkey
        )
        m1_uncal_wfe = remove_PTT(
            m1_uncal_wfe, optics.m1_aperture.transmission.astype(bool)
        )  # Remove PTT from aperture
        m1_uncal_wfe = m1_uncal_wfe * (
            params.get("m1_uncalibrated_amplitude")
            / nanrms(m1_uncal_wfe[optics.m1_aperture.transmission.astype(bool)])
        )  # Scale the 1/f noise map over the aperture
        ...
        # Now for the Secondary
        rng_key, subkey = jr.split(rng_key)
        m2_uncal_wfe = oneoverf_noise_2D(
            optics.wf_npixels, params.get("m2_uncalibrated_power_law"), key=subkey
        )
        m2_uncal_wfe = remove_PTT(
            m2_uncal_wfe, optics.m2_aperture.transmission.astype(bool)
        )  # Remove PTT from aperture
        m2_uncal_wfe = m2_uncal_wfe * (
            params.get("m2_uncalibrated_amplitude")
            / nanrms(m2_uncal_wfe[optics.m2_aperture.transmission.astype(bool)])
        )  # Scale the 1/f noise map over the aperture
...
class SheraTwoPlane_Model(dl.Telescope):
    ...
    def __init__(self, params=None):
        ...
        # Initialize the detector (no jitter for now)
        detector = dl.LayeredDetector(layers=[("downsample", dl.Downsample(optics.oversample))])
    ...
    def _initialize_optics(self, params):
        ...
        cal_wfe = remove_PTT(
            cal_wfe, optics.aperture.transmission.astype(bool)
        )  # Remove PTT from aperture
        cal_wfe = cal_wfe * (
            params.get("calibrated_amplitude")
            / nanrms(cal_wfe[optics.aperture.transmission.astype(bool)])
        )  # Scale the 1/f noise map over the aperture
        ...
        uncal_wfe = remove_PTT(
            uncal_wfe, optics.aperture.transmission.astype(bool)
        )  # Remove PTT from aperture
        uncal_wfe = uncal_wfe * (
            params.get("uncalibrated_amplitude")
            / nanrms(uncal_wfe[optics.aperture.transmission.astype(bool)])
        )  # Scale the 1/f noise map over the aperture
    ...
    def extract_params(self):
        """
        Extract the current parameters from this SheraTwoPlane_Model instance.

        Returns
        -------
        SheraTwoPlaneParams
            A new SheraTwoPlaneParams object populated with the current model parameters.
        """
