"""Run a focused SGD sweep for observation sub-block inference.

This script is intentionally narrow: it tunes SGD hyperparameters for the
...
want a small, repeatable table for choosing SGD settings. The expected active
frame keys are:
...
question by changing only the optimizer/objective fields needed for the SGD
...
GENERATOR_ID = "examples/scripts/sweep_obs_subblock_sgd.py"
MANIFEST_SCHEMA_VERSION = "obs_subblock_sgd_sweep_manifest.v1"
...
DEFAULT_BASE_LRS = (0.25, 0.5, 0.75, 1.0, 1.5, 2.0)
DEFAULT_MOMENTUMS = (0.0, 0.5, 0.8, 0.9)
DEFAULT_NESTEROV_VALUES = (False, True)
DEFAULT_TAIL_K = 10
...
class SGDSweepPoint:
    """One explicit SGD hyperparameter point in the staged sweep grid."""
    ...
    @property
    def optimizer_kwargs(self) -> dict[str, float | bool]:
        return {"momentum": self.momentum, "nesterov": self.nesterov}

    @property
    def run_id(self) -> str:
        return (
            f"sgd_lr{_float_token(self.base_lr)}"
            f"_mom{_float_token(self.momentum)}"
            f"_nest{_bool_token(self.nesterov)}"
        )
...
def _parse_bool_list(raw: str, *, label: str) -> tuple[bool, ...]:
    values: list[bool] = []
    ...
    for item in raw.split(","):
        ...
        if stripped in truthy:
            values.append(True)
        elif stripped in falsy:
            values.append(False)
        else:
            raise ValueError(
                f"{label} must be a comma-separated list of bools "
...
def _validate_grid_values(
    *,
    ...
    momentums: Sequence[float],
    nesterov_values: Sequence[bool],
) -> None:
    ...
    if any(
        ...
        for value in momentums
    ):
        raise ValueError("All SGD momentum values must satisfy 0 <= momentum < 1.")
    if not nesterov_values:
        raise ValueError("nesterov_values must contain at least one value.")
    if any(not isinstance(value, bool) for value in nesterov_values):
        raise ValueError("All nesterov values must be bools.")


def build_sgd_grid(
    *,
    base_lrs: Sequence[float] = DEFAULT_BASE_LRS,
    momentums: Sequence[float] = DEFAULT_MOMENTUMS,
    nesterov_values: Sequence[bool] = DEFAULT_NESTEROV_VALUES,
) -> list[SGDSweepPoint]:
    """Build the small staged SGD grid requested for the current toy problem."""

    _validate_grid_values(
        base_lrs=base_lrs,
        momentums=momentums,
        nesterov_values=nesterov_values,
    )
    points: list[SGDSweepPoint] = []
...
def patch_config_for_sgd_point(
...
def _as_bool(value: Any) -> bool:
    ...
    if isinstance(value, str):
        ...
        if normalized in {"false", "f", "no", "n", "0", "off"}:
            return False
    raise ValueError(f"Expected a bool-like value, got {value!r}.")
...
def recommendation_from_ranked_rows(
    ...
    optimizer = {
        ...
        "kwargs": {
            "momentum": float(best["optimizer.kwargs.momentum"]),
            "nesterov": _as_bool(best["optimizer.kwargs.nesterov"]),
        },
    }
    return {
        "run_id": best["run_id"],
        ...
        "summary": (
            "Best tested SGD configuration for the current three-frame "
...
def _write_recommendation_files(
    *,
    ...
    if recommendation is None:
        text = "# SGD Sweep Recommendation\n\nNo completed, rankable SGD run was found.\n"
    else:
        opt = recommendation["optimizer"]
        kwargs = opt["kwargs"]
        metrics = recommendation["metrics"]
        text = (
            "# SGD Sweep Recommendation\n\n"
            ...
            "  kwargs:\n"
            f"    momentum: {kwargs['momentum']}\n"
            f"    nesterov: {str(kwargs['nesterov']).lower()}\n"
            "objective:\n"
...
def _plot_metric_vs_lr(
    *,
    ...
    groups: dict[tuple[float, bool], list[dict[str, Any]]] = {}
    for row in rows:
        ...
        if not (math.isfinite(value) and math.isfinite(lr)):
            continue
        key = (
            float(row["optimizer.kwargs.momentum"]),
            _as_bool(row["optimizer.kwargs.nesterov"]),
        )
    ...
    for (momentum, nesterov), group_rows in sorted(groups.items()):
        ...
        ax.plot(
            ...
            marker="o",
            linewidth=1.2,
            label=f"momentum={momentum:g}, nesterov={str(nesterov).lower()}",
        )
    ax.set_xscale("log")
    ...
    *,
    ...
    manifest = {
        ...
        "grid": {
            ...
            "momentums": sorted({point.momentum for point in grid}),
            "nesterov_values": sorted({point.nesterov for point in grid}),
            "run_count": len(grid),
        },
        "metric_definitions": {
            "final_truth_score": (
...
def _load_inference_recipe():
    ...
    spec = importlib.util.spec_from_file_location("observation_subblock_inference_for_sgd_sweep", recipe_path)
...
def _base_result_row(
    *,
    ...
    return {
        ...
        "optimizer.kwargs.momentum": point.momentum,
        "optimizer.kwargs.nesterov": point.nesterov,
        "objective.frame_reduce": "mean",
        "objective.subblock_reduce": "sum",
...
def run_sweep(
    *,
    ...
    """Run the staged SGD sweep and return aggregate artifact paths."""
    ...
    experiment_dir = results_dir / (experiment_name or f"sgd_sweep_{timestamp_tag()}")
    ...
    for index, point in enumerate(run_grid, start=1):
        ...
        patched_cfg = patch_config_for_sgd_point(
            base_cfg,
            point,
            runs_root=runs_root,
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Run a focused SGD hyperparameter sweep for the current "
    ...
    parser.add_argument(
        "--results-dir",
        type=Path,
        default=Path("Results/obs_subblock_sgd_sweeps"),
    ...
    parser.add_argument(
        ...
        help="Comma-separated SGD base_lr values.",
    )
    parser.add_argument(
        "--momentums",
        type=str,
        default=",".join(f"{value:g}" for value in DEFAULT_MOMENTUMS),
        help="Comma-separated SGD momentum values.",
    )
    parser.add_argument(
        "--nesterov-values",
        type=str,
        default=",".join(str(value).lower() for value in DEFAULT_NESTEROV_VALUES),
        help="Comma-separated SGD nesterov bool values, for example true,false.",
    )
    parser.add_argument(
        "--max-runs",
        type=int,
...
def main(argv: list[str] | None = None) -> dict[str, Any]:
    ...
    grid = build_sgd_grid(
        ...
        momentums=_parse_float_list(args.momentums, label="--momentums"),
        nesterov_values=_parse_bool_list(
            args.nesterov_values,
    ...
    if result["dry_run"]:
        print(f"Dry run: planned {result['run_count']} SGD runs.")
        print(f"Output directory: {result['output_dir']}")
        for run_id in result["run_ids"]:
            print(f"  {run_id}")
    else:
        print(f"Wrote SGD sweep outputs under: {result['output_dir']}")
        recommendation = result.get("recommendation")
        if recommendation is not None:
            print("Recommended SGD configuration:")
