...
from dluxshera.inference.observation_summary import load_subblock_summary_artifact_payload
...
class PriorContext:
    """Resolved prior initialization context for synthetic or real summary mode."""
...
def build_prior_store_from_system_cfg(
    *,
    ...
    """Build a refreshed forward store from an already-resolved system mapping."""
...
def _require_identical_summary_theta_labels(
    summaries: Sequence[SubblockSummary],
) -> tuple[str, ...]:
    """Require identical Theta label sets for real-summary aggregation."""
...
def _summary_theta_ref_compatibility(
    summaries: Sequence[SubblockSummary],
) -> dict[str, Any]:
    """Summarize agreement or spread across per-summary Theta linearization points."""

    labels = _require_identical_summary_theta_labels(summaries)
    theta_ref_matrix = np.vstack([np.asarray(summary.theta_ref, dtype=float) for summary in summaries])
...
def resolve_prior_context_for_summaries(
    ...
    *,
    ...
    """Resolve the default prior-mean context for one real-summary batch.

    Resolution order:

    1. explicit ``config`` / ``system_preset`` override;
    2. summary ``theta_ref`` values;
    3. resolved system context serialized into the summary artifact;
    4. default preset fallback with a loud provenance warning.
    """

    if not summaries:
        raise ValueError("resolve_prior_context_for_summaries requires at least one summary.")
    labels = _require_identical_summary_theta_labels(summaries)
    compatibility = _summary_theta_ref_compatibility(summaries)
    payloads = [load_subblock_summary_artifact_payload(path) for path in summary_paths]
    warnings = list(compatibility["warnings"])
    explicit_override_requested = explicit_config_path is not None or explicit_system_preset is not None
    ...
    if explicit_override_requested:
        ...
        source_name = "explicit_prior_config" if explicit_config_path is not None else "explicit_prior_system"
    ...
    if prior_source == "summary_theta_ref" or (
        prior_source == "auto"
        and allow_summary_theta_ref_default
    ):
        prior_mean = np.asarray(summaries[0].theta_ref, dtype=float)
        if not np.all(np.isfinite(prior_mean)):
            raise ValueError(
                "Real-summary default prior initialization requires finite theta_ref values."
            )
        provenance = {
            "prior_mean_source": "summary_theta_ref",
            "summary_paths": [str(path) for path in summary_paths],
            "summary_theta_ref_compatibility": compatibility,
            "recommended_prior_context": payloads[0].get("prior_context"),
        }
        return PriorContext(
            theta_labels=labels,
    ...
    *,
    ...
    for index, label in enumerate(labels):
        row = {
            ...
            "prior_mean_source": str(prior_mean_source),
...
def run_observation_belief_update_demo(
    *,
    ...
    if use_external_summaries:
        ...
        summary_labels = _require_identical_summary_theta_labels(summaries)
        layout = ObservationThetaLayout(
    ...
    if use_external_summaries:
        ...
        effective_system_preset = (
            DEFAULT_SYSTEM_PRESET if system_preset is None else system_preset
        )
        ...
        prior_context = PriorContext(
            theta_labels=tuple(layout.labels),
    ...
    summary_payload = {
        ...
        "diagnostics": {
            ...
            "prior_initialization_policy": (
                "Real-summary mode defaults the prior mean to summary theta_ref "
                "unless the user explicitly overrides the prior context. "
                "Synthetic mode derives its prior mean from a resolved system store."
            ),
        },
    }
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Optional YAML/JSON config with a top-level system block. In real-summary "
            "mode this acts as an explicit prior-context override."
        ),
    )
    parser.add_argument(
        "--system-preset",
        ...
        default=None,
        help=(
            "Optional system preset used to derive the prior mean store. "
            "Synthetic mode falls back to SHERA_FLIGHT_3P when omitted. "
            "In real-summary mode this acts as an explicit prior-context override."
        ),
    )
    parser.add_argument(
        "--prior-source",
        ...
        default="auto",
        help=(
            "Prior-mean initialization policy. In real-summary mode the default "
            "'auto' policy uses summary theta_ref before any system-store fallback."
        ),
    )
    parser.add_argument(
        "--n-subblocks",
