"""
Run the two-plane astrometry retrieval recipe.

This file is a thin entrypoint that executes the two-plane astrometry *recipe*
script (the read-first, top-to-bottom workflow). Use this runner when you want
"just run it" execution without hiding the explicit recipe logic.
...
- This runner is intentionally minimal and typically does not expose many CLI
  ...
  disabling eigenmodes), edit the options block near the top of the two-plane
...
def load_recipe():
    recipe_path = Path(__file__).resolve().parents[1] / "recipes" / "twoplane_astrometry.py"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the two-plane astrometry recipe")
