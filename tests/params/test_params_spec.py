...
def _toy_spec() -> ParamSpec:
    return ParamSpec(
        (
            ...
            ParamField(key="optics.primary.zernike_coeffs_nm", group="primary", kind="primitive", shape=(3,)),
...
def test_param_spec_without_basic_complement():
    ...
    assert list(excluded.keys()) == ["a", "optics.primary.zernike_coeffs_nm", "c"]
...
def test_param_spec_without_vector_field_removes_whole_field():
    spec = _toy_spec()
    excluded = spec.without(["optics.primary.zernike_coeffs_nm"])
    assert "optics.primary.zernike_coeffs_nm" not in excluded
