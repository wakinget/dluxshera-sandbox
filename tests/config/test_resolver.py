...
def test_as_dict_accepts_system_or_experiment_only():
    ...
    only_experiment = as_dict({"experiment": {"preset": "INFERENCE_CANONICAL"}})
    both = as_dict(
        {
    ...
    assert set(both) == {"system", "experiment"}

    with pytest.raises(TypeError, match="Config must be a mapping"):
        as_dict(_CfgDC(experiment={"preset": "INFERENCE_CANONICAL"}))
    with pytest.raises(TypeError, match="Config must be a mapping"):
        as_dict(_CfgObj(system={"preset": "TEST_PRESET"}, experiment={"preset": "X"}))
...
def test_missing_required_key_errors(preset_dirs):
    ...
    assert resolved_experiment["kind"] == ""
...
def test_unknown_key_warning_includes_dotted_path(preset_dirs):
    ...
    assert not any("experiment.bar" in msg for msg in messages)


def test_resolver_preserves_numeric_strings_for_consumers(preset_dirs):
    system_dir, _ = preset_dirs
    ...
    assert resolved["source"]["wavelength_m"] == "5.6e-7"
    assert isinstance(resolved["source"]["bandwidth_m"], float)
    assert resolved["source"]["n_lambda"] == "7"
    assert resolved["optics"]["psf_npix"] == "300"
