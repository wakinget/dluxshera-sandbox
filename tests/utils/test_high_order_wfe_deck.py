...
def test_full_fidelity_template_builds_default_deck() -> None:
    ...
    assert deck.primary.full_truth.rms_nm == 20.0
    assert deck.secondary.full_truth.rms_nm == 20.0
    assert deck.primary.provenance["high_order_error_rms_nm"] == 0.3
    assert deck.secondary.provenance["low_order_sigma_nm_per_coeff"] == 2.0
