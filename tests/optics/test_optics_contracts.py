...
def test_threeplane_contract_structural_and_bindings():
    ...
    assert all(key.startswith("optics.") for key in contract.keys())
    assert all(field.group == "optics" for field in contract.values())

    assert contract.get("optics.pupil_npix").structural is True
    assert contract.get("optics.m1_focal_length_m").structural is True
    assert contract.get("optics.dp_path").structural is True
    assert contract.get("optics.throughput").structural is False

    assert contract.get("optics.primary.zernike_coeffs_nm").binding == "p1_layers.m1_aperture.coefficients"
    assert contract.get("optics.secondary.zernike_coeffs_nm").binding == "p2_layers.m2_aperture.coefficients"
    assert contract.get("optics.plate_scale_as_per_pix").binding == "psf_pixel_scale"
...
def test_twoplane_contract_structural_and_bindings():
    ...
    assert contract.get("optics.pupil_npix").structural is True
    assert contract.get("optics.primary_noll_indices").structural is True
    assert contract.get("optics.plate_scale_as_per_pix").structural is False

    assert contract.get("optics.primary.zernike_coeffs_nm").binding == "layers.aperture.coefficients"
    assert contract.get("optics.plate_scale_as_per_pix").binding == "psf_pixel_scale"
...
def test_runtime_binding_updates_cached_optics_fields():
    ...
    plate_scale = forward_store.get("optics.plate_scale_as_per_pix")
    n_m1 = len(cfg.primary_noll_indices)

    store_a = forward_store.replace(
        {
            "optics.primary.zernike_coeffs_nm": jnp.zeros(n_m1),
            "optics.plate_scale_as_per_pix": plate_scale,
        }
    )
    store_b = forward_store.replace(
        {
            "optics.primary.zernike_coeffs_nm": jnp.ones(n_m1),
            "optics.plate_scale_as_per_pix": plate_scale + 1e-3,
        }
    )
