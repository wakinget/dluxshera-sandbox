...
def test_audit_script_writes_review_artifacts(tmp_path: Path) -> None:
    ...
    assert audit["config_kind"] == "full_fidelity_binary_iterative"
...
def test_audit_smoke_uses_explicit_small_spectral_grid_without_fast(tmp_path: Path) -> None:
    ...
    assert summary["configured_truth_n_lambda"] == 3
    assert summary["effective_truth_n_lambda"] == 3
...
def test_audit_identifies_truth_reference_mismatch_fields(tmp_path: Path) -> None:
    ...
    assert "experiment.spectral_model.truth/inference.n_lambda" not in audit["truth_reference_mismatch_fields"]
    assert "experiment.high_order_wfe.truth vs inference.knowledge_error" in audit["truth_reference_mismatch_fields"]


def test_audit_identifies_synthetic_spectral_mismatch_fields(tmp_path: Path) -> None:
    module = load_module()
    raw = yaml.safe_load(CONFIG_PATH.read_text(encoding="utf-8"))
    raw["experiment"]["spectral_model"]["truth"]["n_lambda"] = 5
    raw["experiment"]["spectral_model"]["inference"]["n_lambda"] = 3
    synthetic = tmp_path / "synthetic_spectral_mismatch.yaml"
    synthetic.write_text(yaml.safe_dump(raw), encoding="utf-8")
