...
def test_hpc_sbatch_templates_are_parameterized() -> None:
    for name, default_workers, resource_time in (
        ...
        for expected in (
            ...
            '--profile-runtime --profile-runtime-detail "$PROFILE_RUNTIME_DETAIL"',
            "--memory-diagnostics",
            "python examples/scripts/run_full_fidelity_binary_iterative_campaign.py",
            "--config",
            "--run-name",
            "--results-root",
            "--max-workers",
