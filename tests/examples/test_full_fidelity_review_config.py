...
def test_review_config_has_no_spectral_fast_and_builds(tmp_path: Path) -> None:
    ...
    assert ctx["truth_system_cfg"]["source"]["n_lambda"] == 31
    assert ctx["inference_system_cfg"]["source"]["n_lambda"] == 7
...
def test_review_structured_noise_translates_to_legacy_with_metadata(tmp_path: Path) -> None:
    ...
    assert sub["noise_model"]["legacy_runner_flag"] == "enabled"
    assert sub["noise_model"]["separate_term_control"] is False
