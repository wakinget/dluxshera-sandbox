...
This document captures the history of dLuxShera's transition to the current ParamSpec/ParameterStore/Binder architecture.
The legacy SystemGraph scaffold has since been removed from the codebase.
...
- **Binder/SystemGraph introduction:** Created Binder as the primary model object with a backing SystemGraph to make execution order explicit and testable (SystemGraph later removed).
...
- **After:** ParamSpec and ParameterStore define parameters and values, DerivedResolver handles derivations via a registry, and Binder orchestrates execution with clear, testable flows.
