...
# Source Settings
star_contrasts = np.arange(0.1, 1.1, 0.1)

# Zernike OPD Settings
model_noll_ind = np.arange(4,38) # List of Zernikes present in the model
data_noll_ind = np.arange(4,38) # List of Zernikes present in the data
...
n_sims = star_contrasts.size
for sim_i in np.arange(n_sims):
    starting_seed = seed

    star_contrast = star_contrasts[sim_i]
    ...
    source = dlT.AlphaCen(n_wavels=5, separation=10, position_angle=90, log_flux=total_integrated_log_flux, contrast=star_contrast)  # creating Alpha Centauri source
    ...
    if save_results:
        ...
        if sim_i == 0: # Create a new file for the first simulation
            ...
            with pd.ExcelWriter(os.path.join(save_path, save_name), engine="openpyxl", mode="a", if_sheet_exists="overlay") as writer:
                final_results_df.to_excel(writer, index=False, header=False, sheet_name="Results", startrow=int(sim_i)+1)

        print(f"Simulation {sim_i+1} results saved to {save_name}")


    t1_simulation = time.time()
    print("Simulation %d finished in %.3f sec" % (sim_i+1, t1_simulation-t0_simulation))
