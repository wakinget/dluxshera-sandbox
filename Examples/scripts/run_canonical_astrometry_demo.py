...
```
python -m Examples.scripts.run_canonical_astrometry_demo [--fast] [--save-plots] [--add-noise] [--save-plots-dir PATH]
```
...
- ``--save-plots-dir``: custom output directory for generated plots (implies ``--save-plots``).
...
def _evaluate_psf(binder: SheraThreePlaneBinder, store: Optional[ParameterStore] = None) -> np.ndarray:
    """Evaluate a PSF from the binder given an optional store override."""
...
def _build_param_history(
    ...
    *,
    ...
    """Unpack a θ sequence into per-parameter histories for plotting."""
...
def main(
    ...
    pupil_npix = 64 if fast else 256
    psf_npix = 64 if fast else 256
    ...
    if run_eigen:
        ...
        param_history_eigen = _build_param_history(theta_history_eigen, init_store, sub_spec)
