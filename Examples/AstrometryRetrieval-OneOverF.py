...
def nanrms(arr): # Returns the rms value of the input array, ignoring nans
    return np.nanmean(arr**2)**0.5


# # Function to save results to a CSV file
# def save_results_csv(iteration, param1, param2, result):
#     verbose = True
...
#             print("Created new file and added headers.")
#
#         # Write the results
#         writer.writerow([iteration, param1, param2, result])
#         print(f"Appended results for iteration {iteration}.")


# Start script timer
t0_script = time.time()

# We want to construct a basic optical system using the JNEXT point design, along with some Zernike aberrations.
# We also create the AlphaCen Source object that we want to model.
...
model_noll_ind = np.arange(4,37)
data_noll_ind = np.arange(4,37)
...
# Add 1/f Noise to the data, exclude from the model
oneoverf_amp_rms = 1e-9
oneoverf_alpha = 2.5
oneoverf_opd_true = oneoverf_noise_2D(model_optics.wf_npixels, oneoverf_alpha, key=keys[0])
...
source = dlT.AlphaCen(n_wavels=5, separation=10, position_angle=90, log_flux=total_integrated_log_flux, contrast=1.0)  # creating Alpha Centauri source
...
zCoeff_amp_rms = 0
zCoeffs_true = zCoeff_amp_rms * jr.normal(keys[1], data_model.coefficients.shape)
...
add_shot_noise = True
if add_shot_noise:
    photons = jr.poisson(keys[2], data_psf) # Add photon noise
else:
    photons = data_psf
...
read_noise = sigma_read * np.sqrt(N_frames) * jr.normal(keys[3], data_psf.shape)
...
# coords = dlu.pixel_coords(5 * model_optics.wf_npixels, model_optics.diameter)
# outer = dlu.circle(coords, model_optics.diameter / 2)
# m1_mask = dlu.combine([outer], 5)
...
# zernike_opd_true_rms_nm = 1e9* np.nanmean((zernike_opd_true[ap_mask.astype(bool)])**2 )**0.5
...
# print("Detector Platescale: %.3f as/pix" % platescale_true)
# print("Data Includes %.3f nm rms of 1/f^%.2f noise" % (oneoverf_amp_rms*1e9, oneoverf_alpha))
# print("Data Includes %d Zernikes: Z%d - Z%d @%.2f nm rms" % (nZern_data, 4, 3+nZern_data, zCoeff_amp_rms))
# print("Data Includes Shot Noise: %s" % add_shot_noise)
# print("Data Includes Read Noise @ %.2f e- per frame" % (sigma_read/np.sqrt(N_frames)))
...
if save_results:
    ...
    progression_results = {
        "Iteration": np.arange(n_iter) + 1,
        "Losses": losses,
        "Found Source Position X (as)": posX_found,
        "Found Source Position Y (as)": posY_found,
        "Found Source Separation (as)": sep_found,
        "Found Source Angle (deg)": posAng_found,
        "Found Source Log Flux": logFlux_found,
        "Found Source Contrast (A:B)": contrast_found,
        "Found Source A Raw Flux": rawFlux_found[:, 0],
        "Found Source B Raw Flux": rawFlux_found[:, 1],
        "Found Platescale (as/pixel)": platescale_found,
        "Found Zernike Coefficient Amplitudes (nm)": [", ".join(map(str, row)) for row in zCoeffs_found],

        "Residual Source Position X (uas)": posX_resid*1e6,
        "Residual Source Position Y (uas)": posY_resid*1e6,
        "Residual Source Separation (uas)": sep_resid*1e6,
        "Residual Source Angle (as)": posAng_resid*60**2,
        "Residual Source A Flux (ppm)": rawFlux_fracResid[:, 0] * 1e6,
        "Residual Source B Flux (ppm)": rawFlux_fracResid[:, 1] * 1e6,
        "Residual Platescale (ppm)": platescale_resid / platescale_true * 1e6,
        "Residual Zernike RMS Error (nm)": zernike_opd_resid_rms_nm
    }


    # Construct Data Frames
    final_results_df = pd.DataFrame([final_results])
    progression_results_df = pd.DataFrame(progression_results)

    # Construct Save Name
    save_name = f"{script_name}_{timestamp}.xlsx"

    # Write to an Excel file
    with pd.ExcelWriter(os.path.join(save_path, save_name), engine="xlsxwriter") as writer:
        # Save final_results to the first sheet
        final_results_df.to_excel(writer, sheet_name="Final Results", index=False)
        # Save progression_results to the second sheet
        progression_results_df.to_excel(writer, sheet_name="Progression Results", index=False)
