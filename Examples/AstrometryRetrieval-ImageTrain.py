...
print2console_iter = False
print2console_final = True

# RNG Settings
starting_seed = 0
# seeds = np.arange(20)

# Pointing Error Settings - Changes each frame
pointing_error_rms_amplitude = 0 # as, in X and Y

# Source Settings
star_contrast = 2.78
initial_separation = 10.0 # as
initial_angle = 90.0
initial_center = (0, 0)
central_wavelength = 550. # nm
bandwidth = 110. # nm
n_wavelengths = 5
delta_separation = 0e-6 # as
...
# Exposure Settings
exposure_per_frame = 0.05  # seconds
total_exposure = 1800 # seconds
N_frames = int(np.ceil(total_exposure/exposure_per_frame)) # frames
...
total_exposure = exposure_per_frame  # seconds
aperture_area = np.pi * (model_optics.diameter / 2) ** 2  # square meters of aperture (doesn't include M2 obscuration)
bandpass = (central_wavelength-bandwidth/2, central_wavelength+bandwidth/2)

total_integrated_flux = default_total_flux * total_exposure * aperture_area * (bandwidth/1000)  # photons
...
oneoverf_opd_true = oneoverf_noise_2D(model_optics.wf_npixels, oneoverf_alpha, key=subkey)
...
frame_keys = jr.split(rng_key, N_frames)
param_history = dict([(param, []) for param in params])
flux_history = []
for frame_i in np.arange(N_frames):
    frame_key = frame_keys[frame_i]

    # Simulate a new pointing error
    frame_key, subkey = jr.split(frame_key)
    pointing_err_xy_as = pointing_error_rms_amplitude* jr.normal(subkey, (2,))
    ...
    # Simulate change in separation
    # frame_key, subkey = jr.split(frame_key)
    # deltaSep_true = initial_separation*separation_change_ppm_rms*1e-6 * jr.normal(subkey)
    perturbed_separation_as = initial_separation + delta_separation
    data_model = data_model.set('source.separation', perturbed_separation_as)
    ...
    if add_shot_noise:
        frame_key, subkey = jr.split(frame_key)
        if total_exposure >= 7200: # If exposure is > 2 hours
    ...
    # Add Read Noise
    frame_key, subkey = jr.split(frame_key)
    read_noise = sigma_read * jr.normal(subkey, data_psf.shape)
    ...
    if print2console_iter:
        ...
        print("Separation Change: %.3f uas" % (delta_separation*1e6))
    ...
    plt.tight_layout()
    if save_plots:
        frame_digits = len(str(N_frames))
        plot_name = "DataInput"
        save_name = f"{script_name}_{plot_name}_{timestamp}_Frame{frame_i+1:0{frame_digits}d}.png"
    ...
    keys = jr.split(frame_key, len(leaves))
    ...
    flux_history.append(models_out[-1].get('source.raw_fluxes'))
    ...
    zCoeffs_std = err_model.coefficients
    ...
    if print2console_iter:
        ...
        print("zCoeffs STD RMS: %.3f nm" % (nanrms(zCoeffs_std)))
    ...
    if save_results:
        ...
        final_results = {
            ...
            "Input Pointing Error X (as)": pointing_err_xy_as[0],
            "Input Pointing Error Y (as)": pointing_err_xy_as[1],
            "Input Separation Change (uas)": (delta_separation * 1e6),
        ...
        if frame_i == 0: # Create a new file for the first simulation
            ...
            with pd.ExcelWriter(os.path.join(save_path, save_name), engine="openpyxl", mode="a", if_sheet_exists="overlay") as writer:
                final_results_df.to_excel(writer, index=False, header=False, sheet_name="Results", startrow=int(frame_i)+1)

        print(f"Frame {frame_i+1}/{N_frames} results saved to {save_name}")


t1_simulation = time.time()
print("Simulation %d finished in %.3f sec" % (frame_i+1, t1_simulation-t0_simulation))

# Format outputs
flux_history = np.array(flux_history)
posX_found = np.array([ind for ind in param_history['x_position']])
posY_found = np.array([ind for ind in param_history['y_position']])
sep_found = np.array([ind for ind in param_history['separation']])
posAng_found = np.array([ind for ind in param_history['position_angle']])
logFlux_found = np.array([ind for ind in param_history['log_flux']])
contrast_found = np.array([ind for ind in param_history['contrast']])
platescale_found = np.array([ind for ind in param_history['psf_pixel_scale']])
zCoeffs_found = np.array([ind for ind in param_history['coefficients']])

posX_mean = np.mean(posX_found)
posY_mean = np.mean(posY_found)
sep_mean = np.mean(sep_found)
posAng_mean = np.mean(posAng_found)
# logFlux_mean = np.mean(logFlux_found)
# contrast_mean = np.mean(contrast_found)
platescale_mean = np.mean(platescale_found)
# zCoeffs_mean = np.mean(zCoeffs_found)
rawFlux_mean = np.mean(flux_history, axis=0)

posX_resid_uas = (posX_mean - initial_center[0])*1e6
posY_resid_uas = (posY_mean - initial_center[1])*1e6
# center_residual_uas = np.array([posX_mean - initial_center[0], posY_mean - initial_center[1]])*1e6
sep_resid_uas = (sep_mean - sep_true)*1e6
posAng_resid_as = (posAng_mean - posAng_true)*60**2
rawFlux_resid_ppm = (rawFlux_mean - rawFlux_true)/rawFlux_true*1e6
platescale_resid_ppm = (platescale_mean - platescale_true)/platescale_true*1e6

posX_sem_uas = np.std(posX_found, axis=0, ddof=1) / np.sqrt(N_frames) *1e6
posY_sem_uas = np.std(posY_found, axis=0, ddof=1) / np.sqrt(N_frames) *1e6
sep_sem_uas = np.std(sep_found, axis=0, ddof=1) / np.sqrt(N_frames) *1e6
posAng_sem_as = np.std(posAng_found, axis=0, ddof=1) / np.sqrt(N_frames) *60**2
# logFlux_sem = np.std(logFlux_found, axis=0, ddof=1) / np.sqrt(N_frames)
# contrast_sem = np.std(contrast_found, axis=0, ddof=1) / np.sqrt(N_frames)
# platescale_sem = np.std(platescale_found, axis=0, ddof=1) / np.sqrt(N_frames)

if print2console_final:
    # Print Final Results
    print("Astrometry Retrieval Image Train Final Results:")
    print("%d frames in %.3f sec" % (N_frames, t1_simulation-t0_simulation))

    print("Initial X, Y Position: %.3f as, %.3f as" % initial_center)
    print("Pointing Error: %.3f as rms" % pointing_error_rms_amplitude)
    print("Residual Error, Position X: %.3f uas +/- %.3f uas" % (posX_resid_uas, posX_sem_uas))
    print("Residual Error, Position Y: %.3f uas +/- %.3f uas" % (posY_resid_uas, posY_sem_uas))

    print("Initial Separation: %.3f as" % initial_separation)
    print("True Separation Change: %.3f uas" % (delta_separation*1e6))
    print("Residual Error, Separation: %.3f uas +/- %.3f uas" % (sep_resid_uas, sep_sem_uas))
