...
from jax import jit, grad, linearize, lax, config, tree, Array
...
def propagate_fresnel_AS(
    ...
    Parameters:
        Ein       : Input field (Complex 2D Array). The (0, 0) element is on-axis.
                    Use np.fft.fftshift if necessary before calling.
                    Size of the array must be even, and ideally a power of 2.
        wavelength: Wavelength of light.
        radius    : Radius of the aperture.
        prop_dist : Distance to propagate.
        pad       : int = 2. Amount to pad the input array by before propagation. Note this function
                    does not automatically crop the output.

    Returns:
        Propagated field (complex 2D Array) with the (0, 0) element on-axis.
        Sampling is the same as for Ein.
    """

    # plt.figure(figsize=(5, 4))
    # plt.title("I in")
    # plt.imshow(Ein.real, extent=radius*np.array([-1, 1, -1, 1]))
    # plt.colorbar()
    # plt.xlabel("X (m)")
    # plt.ylabel("Y (m)")
    # plt.show()
    ...
    # Un-pad the array
    # E_propped = E_propped[Npad:-Npad, Npad:-Npad]
...
dprime = -phi_1 / phi_telescope * D # m, Shift of the back principal plane from the plane of M2

# Calculate Magnification
M_1 = 1/(1 - D*phi_1)
BFD = f_telescope + dprime # m, Back Focal Distance, this is the distance between M2 and focus

# Print Summary
print("Telescope Reduction:")
print("Telescope EFL: %.3f m" % f_telescope)
print("Front Principal Plane Shift: d = %.3f m" % d)
print("Back Principal Plane Shift: d' = %.3f m" % dprime)
print("Back Focal Distance: %.3f m" % BFD)
print("Primary Magnification: %.3f" % M_1)
...
N_psf = 512
psf_pixel_scale = dlu.rad2arcsec( dx_detector / f_telescope ) /2
psf_oversamp = 1
optics = dl.AngularOpticalSystem(
    N_pupil, M1_diam, layers, N_psf, psf_pixel_scale, psf_oversamp
)
...
# plt.subplot(1, 2, 1)
# plt.title("Aperture")
# plt.imshow(aperture, extent=M1_extent)
# plt.colorbar()
# plt.xlabel("X (m)")
# plt.ylabel("Y (m)")
# # plt.show()
...
# plt.subplot(1, 2, 2)
# plt.title("Sqrt PSF")
# plt.imshow(psf**0.5, extent=psf_extent)
...
# Use Fresnel_AS to propagate to the secondary
...
wavefront = dl.Wavefront(N_pupil, M1_diam, wavelength)
# offset = np.zeros(2)
...
E_MFT = MFT_wf.phasor
I_MFT = E_MFT * np.conj(E_MFT)

# plt.figure(figsize=(5, 4))
# plt.imshow(layers['aperture'])
# plt.show()
...
# plt.subplot(1,2, 2)
# plt.title("imag( I in )")
# plt.imshow(I_M1.imag, extent=M1_extent)
# plt.colorbar()
# plt.xlabel("X (m)")
# plt.ylabel("Y (m)")
# plt.show()
...
print("Wavelength: %.3f nm" % (wavelength*1e9))
print("Pupil_diameter: %.3f mm" % (M1_diam*1e3))
print("N_Pupil: %d pix" % N_pupil)
print("Pupil_dx: %.3f um/pix" % (M1_diam/N_pupil*1e6))
print("Radius: %.3f mm" % (radius*1e3))
...
# Initialize a Wavefront object for the secondary
M2_wv = dl.Wavefront(2*N_pupil, 2*M1_diam, wavelength)
M2_wv = M2_wv.set('amplitude', np.abs(E_M2))
...
# plt.figure(figsize=(5, 4))
# plt.title("I in")
# plt.imshow(wavefront.amplitude**2, extent=M1_extent)
# plt.colorbar()
# plt.xlabel("X (m)")
# plt.ylabel("Y (m)")
# plt.show()
...
# plt.subplot(1, 3, 1)
# plt.title("SQRT PSF 1")
# plt.imshow(I_MFT.real**0.5, extent=psf_extent)
# plt.colorbar()
# plt.xlabel("X (as)")
# plt.ylabel("Y (as)")
# plt.subplot(1, 3, 2)
...
# plt.subplot(1, 3, 3)
# plt.title("PSF Difference")
# plt.imshow(np.abs(I_MFT.real**0.5 - I_focus.real**0.5), extent=psf_extent)
# plt.colorbar()
# plt.xlabel("X (as)")
# plt.ylabel("Y (as)")
# plt.show()
