...
dp_file = 'SHERA_DP_Surface_20250327_182336.fits'

# Zernike OPD Settings
model_noll_ind = np.arange(4,11) # List of Zernikes present in the model
data_noll_ind = np.arange(4,11) # List of Zernikes present in the data
...
focal_length = 1/(1/m1_focal_length + 1/m2_focal_length - m1_m2_separation/m1_focal_length/m2_focal_length)
psf_pixel_scale = dlu.rad2arcsec(detector_pixel_pitch*1e-6/focal_length)
...
# Add layers to the data and model to hold WFE calibration
# wfe_layer = dll.AberratedLayer(np.zeros((model_optics.wf_npixels, model_optics.wf_npixels)))
# model_optics = model_optics.insert_layer(('calibration', wfe_layer), 3)
# data_optics = data_optics.insert_layer(('calibration', wfe_layer), 3)

# Generate a WFE Calibration
rng_key, subkey = jr.split(rng_key0)
wfe_cal_true = oneoverf_noise_2D(model_optics.wf_npixels, calib_alpha, key=subkey)
wfe_cal_true = remove_PTT(wfe_cal_true, model_optics.aperture.transmission.astype(bool))  # Remove PTT from aperture
wfe_cal_true = wfe_cal_true * (calib_amp_rms / nanrms(wfe_cal_true[ap_mask.astype(bool)]))  # Scale the 1/f noise map over the aperture
wfe_cal_layer = dll.AberratedLayer(wfe_cal_true)
model_optics = model_optics.insert_layer(('calibration', wfe_cal_layer), 3)
data_optics = data_optics.insert_layer(('calibration', wfe_cal_layer), 3)
# Calibration built into both data and model


# Add a WFE layer to the data model - this holds the 1/f map later - This represents uncalibrated WFE
rng_key, subkey = jr.split(rng_key)
wfe_uncal_true = oneoverf_noise_2D(model_optics.wf_npixels, wfe_alpha, key=subkey)
wfe_uncal_true = remove_PTT(wfe_uncal_true, model_optics.aperture.transmission.astype(bool))  # Remove PTT from aperture
wfe_uncal_true = wfe_uncal_true * (wfe_amp_rms / nanrms(wfe_uncal_true[ap_mask.astype(bool)]))  # Scale the 1/f noise map over the aperture
wfe_uncal_layer = dll.AberratedLayer(wfe_uncal_true)
data_optics = data_optics.insert_layer(('wfe', wfe_uncal_layer), 4)
# WFE built into the data only
...
for obs_i in np.arange(N_observations):
    ...
    if print2console:
        ...
        print("Data Includes %.3f nm rms of calibrated 1/f^%.2f noise" % (calib_amp_rms*1e9, calib_alpha))
        print("Data Includes %.3f nm rms of uncalibrated 1/f^%.2f noise" % (wfe_amp_rms*1e9, wfe_alpha))
    ...
    plt.title("Calibrated WFE: %.3f nm rms" % (calib_amp_rms * 1e9))
    plt.xlabel("X (mm)")
    im = ax.imshow(1e9 * wfe_cal_true * ap_nanmask, inferno, extent=pupil_extent_mm)
    cbar = fig.colorbar(im, cax=merge_cbar(ax))
    cbar.set_label("nm")
    ax = plt.subplot(1, 4, 2)
    plt.title("Uncalibrated WFE: %.3f nm rms" % (wfe_amp_rms * 1e9))
    plt.xlabel("X (mm)")
    im = ax.imshow(1e9 * wfe_uncal_true * ap_nanmask, inferno, extent=pupil_extent_mm)
    cbar = fig.colorbar(im, cax=merge_cbar(ax))
    cbar.set_label("nm")
    ...
    total_opd_true = zernike_opd_true + wfe_uncal_true
    total_opd_true_rms_nm = 1e9* nanrms(total_opd_true[ap_mask.astype(bool)])
    total_opd_true_tiled = np.tile(total_opd_true, [n_iter, 1, 1])
    ...
    plt.xlabel("X (mm)")
    im = plt.imshow(1e9* total_opd_true * ap_nanmask, inferno, vmin=vmin, vmax=vmax, extent=pupil_extent_mm)
    cbar = fig.colorbar(im, cax=merge_cbar(ax))
    # cbar.set_label("nm", labelpad=0)
    ax = plt.subplot(2, 3, 5)
    plt.title(f"Found OPD: %.3fnm rms" % zernike_opd_found_rms_nm[-1])
    ...
    if save_results:
        ...
        final_results = {
            ...
            "Input Calibration 1/f Amplitude (nm rms)": calib_amp_rms*1e9,
            "Input Calibration 1/f Power Law": calib_alpha,
            "Input Uncalibrated 1/f Amplitude (nm rms)": wfe_amp_rms*1e9,
            "Input Uncalibrated 1/f Power Law": wfe_alpha,
