...
save_path = "/Users/Dmckeith/Documents/Python/Shera/dLuxShera/Results/"
script_name = os.path.splitext((os.path.basename(__file__)))[0]
# script_name = "AstrometryRetrieval-V2"
...
# noll_ind = np.arange(4, 11)
# zCoeffs_true = 1e-9*jr.normal(keys[0], noll_ind.shape)
# optics = JNEXTOpticalSystem(psf_npixels=256, noll_indices=noll_ind, coefficients=zCoeffs_true, oversample=1)  # creating JNEXT optical system
...
optics_ho = dlT.TolimanOpticalSystem(
    wf_npixels=512,
    psf_npixels=128,
    oversample=3,
    radial_orders=[2, 3, 4, 5],
)

# Normalise the aberration basis to be in units of nm
optics = optics.multiply('basis', 1e-9)
optics_ho = optics_ho.multiply('basis', 1e-9)
...
# Create a model for the data that includes higher order aberrations
...
# Examine the Model
pupil = np.where(model.aperture.transmission, model.pupil.opd, np.nan)
...
im = ax.imshow(pupil, inferno, extent=pupil_extent_mm)
...
data_model = data_model.set("coefficients", 1 * jr.normal(keys[0], data_model.coefficients.shape))

# Model the PSF
psf = data_model.model()
photons = jr.poisson(keys[1], psf) # Add photon noise

# Add read noise
sigma_read = 5 * np.sqrt(N_frames)
read_noise = sigma_read * jr.normal(keys[2], psf.shape)
