...
separation_change_ppm = 1 # Separation change to introduce
defocus_amplitude = 1.5 # nm, Z4 amplitude
...
# optax.sgd(vals, nesterov=True, momentum=0.6)
# optimiser_label = "optax.sgd-nesterov-0p6"
...
map_keys = jr.split(rng_key0, n_maps)  # One key per WFE map
for map_i in np.arange(n_maps):
    map_key = map_keys[map_i]

    # Generate new WFE map
    map_key, subkey = jr.split(map_key)
    ...
    pointing_keys = jr.split(map_key, n_pointings)
    for pointing_i in np.arange(n_pointings):
        pointing_key = pointing_keys[pointing_i]
        sim_i += 1
        ...
        pointing_key, subkey = jr.split(pointing_key)
        deltaSep_true = initial_separation*separation_change_ppm*1e-6
        perturbed_separation_as = initial_separation + deltaSep_true
        ...
        zCoeffs_true = zCoeffs_true.at[0].set(defocus_amplitude) # Set the Defocus Term
        ...
        cbar.set_label("Photons")
        plt.tight_layout()
        if save_plots and sim_i <= N_saved_plots:
        ...
        keys = jr.split(pointing_key, len(leaves))
        ...
        if save_results:
            ...
            final_results = {
                ...
                "Input Separation Change (uas)": (deltaSep_true * 1e6),
                "Input Defocus (nm)": defocus_amplitude,
